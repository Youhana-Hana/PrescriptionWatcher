...
public class ChordChannelListener implements IChordChannelListener {
    ...
    public void onFileWillReceive(String fromNode, String fromChannel, String fileName,
        ...
        if (availableMemory < fileSize) {
            ...
            if (null != this.listener)
                this.listener.onFileCompleted(IChordServiceListener.FAILED, fromNode, fromChannel,
                        exchangeId, fileName);
            return;
        }

        if (null != this.listener)
            this.listener.onFileWillReceive(fromNode, fromChannel, fileName, exchangeId);
    }
    ...
    public void onFileChunkReceived(String fromNode, String fromChannel, String fileName,
        ...
        if (null != this.listener) {
            ...
            this.listener.onFileProgress(false, fromNode, fromChannel, progress, exchangeId);
        }
    }
    ...
    public void onFileReceived(String fromNode, String fromChannel, String fileName,
        ...
        File targetFile = new File(this.chordManagerService.getChordFilePath(), savedName);
        int index = 0;
        while (targetFile.exists()) {
            savedName = name + "_" + index + ext;
            targetFile = new File(this.chordManagerService.getChordFilePath(), savedName);
        ...
        if (null != this.listener) {
            this.listener.onFileCompleted(IChordServiceListener.RECEIVED, fromNode, fromChannel,
    ...
    public void onFileChunkSent(String toNode, String toChannel, String fileName, String hash,
        ...
        if (null != this.listener) {
            int progress = (int) (offset * 100 / fileSize);
            this.listener.onFileProgress(true, toNode, toChannel, progress, exchangeId);
        }
    }
    ...
    public void onFileSent(String toNode, String toChannel, String fileName, String hash,
        ...
        if (null != this.listener) {
            this.listener.onFileCompleted(IChordServiceListener.SENT, toNode, toChannel,
                    exchangeId, fileName);
        }
    }
        ...
        switch (reason) {
            case ERROR_FILE_REJECTED: {
                Log.e(TAG, TAGClass + "onFileFailed() - REJECTED Node : " + node
                        + ", fromChannel : " + channel);
                if (null != this.listener) {
                    this.listener.onFileCompleted(IChordServiceListener.REJECTED, node, channel,
                            exchangeId, fileName);
                }
                break;
            }
            ...
            case ERROR_FILE_CANCELED: {
                Log.e(TAG, TAGClass + "onFileFailed() - CANCELED Node : " + node
                        + ", fromChannel : " + channel);
                if (null != this.listener) {
                    this.listener.onFileCompleted(IChordServiceListener.CANCELLED, node, channel,
                            exchangeId, fileName);
                }
                break;
            }
            ...
            default:
                Log.e(TAG, TAGClass + "onFileFailed() - " + reason + " : " + node
                        + ", fromChannel : " + channel);
                if (null != this.listener) {
                    this.listener.onFileCompleted(IChordServiceListener.FAILED, node, channel,
                            exchangeId, fileName);
                }
                break;
        }
